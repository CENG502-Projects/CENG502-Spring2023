...
class MLP(torch.nn.Module):
	...
	def __init__(self, args):
		...
		self.fc5 = torch.nn.Linear(in_features=191, out_features=128) # add skip connection here
		...
		self.fc9 = torch.nn.Linear(in_features=191, out_features=128)
		self.fc10 = torch.nn.Linear(in_features=128, out_features=3)


		self.fc_density = torch.nn.Linear(in_features=128, out_features=1)
	...
	def forward(self, x, d):
		...
		out = torch.cat([encoded_pos, out], dim=-1).to(torch.float32)
		...
		density = self.fc_density(out)

		density = F.relu(density)

		encoded_dir = self.c2f_positional_encoding(input=d, L=self.L)

		encoded_dir = torch.cat([out, d, encoded_dir], dim=-1).to(torch.float32)
